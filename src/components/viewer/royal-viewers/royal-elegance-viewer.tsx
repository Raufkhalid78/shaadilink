...
export default function RoyalEleganceViewer({ templateId, flowData, guestName }: RoyalViewerProps) {
  ...
  return (
    ...
    >
      <RosePetalsBg accent={theme.accent} />
      ...
      {flowData?.showBismillah !== false && (
        <m.div initial={{ opacity: 0 }} animate={s.doorsOpened ? { opacity: 1 } : { opacity: 0 }} transition={{ delay: 2.2, duration: 1.0 }}
          ...
          <m.p
            initial={{ opacity: 0, y: '30vh', scale: 1.15 }}
      ...
      <m.div
        ...
        animate={{ opacity: s.heroVisible ? 1 : 0, scale: s.heroVisible ? 1 : 0.94, filter: s.heroVisible ? 'blur(0px)' : 'blur(6px)', y: s.heroVisible ? 0 : 20 }}
        transition={{ duration: 2.2, ease: [0.16, 1, 0.3, 1] }}
      >

        {/* ─── HERO — Split Layout ─── */}
        <section className="relative min-h-screen flex flex-col items-center justify-center px-4 py-20 overflow-hidden">
          <div className="absolute inset-0" style={{ background: `radial-gradient(ellipse at 50% 60%, rgba(232,48,90,0.08) 0%, transparent 60%)`, backgroundColor: theme.bgPrimary }} />

          <div className="relative z-10 w-full max-w-2xl">
            <m.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="text-center text-xs tracking-[0.5em] uppercase mb-8" style={{ color: theme.textSecondary }}>
              {s.t('gettingMarried', "We're getting married")}
            </m.p>

            {/* Split hero: name & ornate ampersand */}
            <div className="flex items-center justify-center gap-2 sm:gap-6 flex-wrap">
              <m.h1 initial={{ opacity: 0, x: -40 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.5, duration: 0.9, ease: [0.16, 1, 0.3, 1] }}
                className="text-4xl sm:text-5xl md:text-6xl font-bold font-[var(--font-great-vibes)] text-right flex-1 min-w-[140px]"
                style={{ color: theme.textPrimary, textShadow: `0 0 20px ${theme.accent}44` }}>
                {s.translatedPartner1}
              </m.h1>

              <m.div initial={{ opacity: 0, scale: 0 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.7, duration: 0.6, type: 'spring', stiffness: 200 }}
                className="flex-shrink-0 text-6xl sm:text-8xl font-[var(--font-great-vibes)]"
                style={{ color: theme.accent, textShadow: `0 0 30px ${theme.accent}88` }}>
                &
              </m.div>

              <m.h1 initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.5, duration: 0.9, ease: [0.16, 1, 0.3, 1] }}
                className="text-4xl sm:text-5xl md:text-6xl font-bold font-[var(--font-great-vibes)] text-left flex-1 min-w-[140px]"
                style={{ color: theme.textPrimary, textShadow: `0 0 20px ${theme.accent}44` }}>
                {s.translatedPartner2}
              </m.h1>
            </div>

            <m.div initial={{ scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ delay: 1.0, duration: 0.8 }} className="flex items-center justify-center gap-4 my-6">
            ...
            <m.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.4 }} className="text-center text-base tracking-[0.15em] font-[var(--font-great-vibes)]" style={{ color: theme.textSecondary }}>
        ...
        <RevealSection>
          <section className="py-16 md:py-20 px-6" style={{ background: `linear-gradient(180deg, ${theme.bgPrimary} 0%, ${theme.bgSecondary}88 50%, ${theme.bgPrimary} 100%)` }}>
            <div className="max-w-lg mx-auto text-center">
              {s.guestNameFromUrl && (<m.h3 initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-3xl md:text-4xl capitalize mb-8 font-[var(--font-great-vibes)]" style={{ color: theme.accent }}>{s.language === 'ur' ? 'محترم' : 'Dear'} {s.guestNameFromUrl},</m.h3>)}
        ...
        {flowData?.showQuranVerse && (
          <RevealSection>
            <section className="py-16 md:py-20 px-6">
              <div className="max-w-2xl mx-auto text-center space-y-6 py-10 px-6 rounded-2xl border backdrop-blur-md" style={{ borderColor: getOpacityStyle('border', 0.15), backgroundColor: getOpacityStyle('bg', 0.3) }}>
                ...
                <p className="font-arabic text-2xl md:text-3xl leading-loose bismillah-glow" dir="rtl" style={{ color: theme.accentLight }}>وَمِنْ ءَايَـٰتِهِۦٓ أَنْ خَلَقَ لَكُم مِّنْ أَنفُسِكُمْ أَزْوَٰجًا لِّتَسْکُنُوٓا۟ إِلَيْهَا وَجَعَلَ بَيْنَكُم مَّوَدَّةً وَرَحْمَةً</p>
        ...
        {/* ─── VIDEO ─── */}
        {s.youtubeVideoId && (<RevealSection><section className="py-16 md:py-20 px-6"><div className="flex flex-col items-center gap-6"><h2 className="text-3xl sm:text-4xl text-center font-[var(--font-great-vibes)]" style={{ color: theme.accent }}>Our Story</h2><RoseDivider accent={theme.accent} /><div className="w-full max-w-3xl mx-auto rounded-xl overflow-hidden border" style={{ borderColor: getOpacityStyle('border', 0.2) }}><div className="aspect-video"><iframe className="w-full h-full" src={`https://www.youtube.com/embed/${s.youtubeVideoId}?rel=0&modestbranding=1`} title="Our Story" frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen /></div></div></div></section></RevealSection>)}
        ...
        <div className="py-10 text-center border-t" style={{ borderColor: getOpacityStyle('border', 0.1) }}>
          ...
          <p className="text-xs tracking-wider" style={{ color: getOpacityStyle('text', 0.3) }}>{s.t('madeWithLove', 'Made with love by ShaadiLink')}</p>
        </div>
      </m.div>
