    ...
    >
      ...
      >
        ...
        {flowData?.showQuranVerse && (<RevealSection><section className="relative py-16 md:py-20 px-6 overflow-hidden"><AuroraBg accent={theme.accent} /><div className="max-w-2xl mx-auto text-center space-y-6 py-10 px-6 rounded-2xl border relative z-10" style={{ borderColor: getOpacityStyle('border', 0.15), backgroundColor: getOpacityStyle('bg', 0.3), boxShadow: `0 0 30px ${theme.accent}18` }}><div className="flex justify-center items-center gap-4 mb-2"><div className="w-8 h-px" style={{ background: `linear-gradient(90deg, transparent, ${theme.accent})` }} /><span className="text-gold opacity-80 text-2xl font-arabic">﷽</span><div className="w-8 h-px" style={{ background: `linear-gradient(-90deg, transparent, ${theme.accent})` }} /></div><p className="font-arabic text-2xl md:text-3xl leading-loose" dir="rtl" style={{ color: theme.accentLight }}>وَمِنْ ءَايَـٰتِهِۦٓ أَنْ خَلَقَ لَكُم مِّنْ أَنفُسِكُمْ أَزْوَٰجًا لِّتَسْکُنُوٓا۟ إِلَيْهَا وَجَعَلَ بَيْنَكُم مَّوَدَّةً وَرَحْمَةً</p>{flowData?.customVerseText ? (
                  ...
                  <>
                    <p className="text-sm md:text-base italic leading-relaxed" style={{ color: theme.textSecondary }}>&ldquo;And of His signs is that He created for you from yourselves mates that you may find tranquility.&rdquo;<span className="block text-xs mt-2 font-semibold not-italic" style={{ color: theme.accent }}>— Surah Ar-Rum [30:21]</span></p>
                  </>
        ...
        {/* ─── VIDEO ─── */}
        {s.youtubeVideoId && (<RevealSection><section className="relative py-16 md:py-20 px-6 overflow-hidden"><AuroraBg accent={theme.accent} /><div className="flex flex-col items-center gap-6 relative z-10"><h2 className="text-3xl sm:text-4xl text-center font-[var(--font-great-vibes)]" style={{ color: theme.accent, textShadow: `0 0 20px ${theme.accent}66` }}>{s.t('Our Story', 'Our Story')}</h2><StarDivider accent={theme.accent} /><div className="w-full max-w-3xl mx-auto rounded-xl overflow-hidden border" style={{ borderColor: getOpacityStyle('border', 0.2), boxShadow: `0 0 30px ${theme.accent}18` }}><div className="aspect-video"><iframe className="w-full h-full" src={`https://www.youtube.com/embed/${s.youtubeVideoId}?rel=0&modestbranding=1`} title={s.t('Our Story', 'Our Story')} frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen /></div></div></div></section></RevealSection>)}
