            ...
            <div className="max-w-lg mx-auto text-center relative z-10">
              {s.guestNameFromUrl && (
                <m.div initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="flex flex-col items-center gap-4 mb-8">
                  <h3 className="text-3xl md:text-4xl capitalize font-[var(--font-great-vibes)]" style={{ color: theme.accent, textShadow: `0 0 20px ${theme.accent}66` }}>{s.language === 'ur' ? 'محترم' : 'Dear'} {s.guestNameFromUrl},</h3>
                          ...
                          <div className="flex-1">
                            <ConstellationCard accent={theme.accent} bg={getOpacityStyle('bg', 0.05)} border={theme.borderSubtle}>
                              ...
                              {flowData?.showNikahRegistration && (event.name.toLowerCase().includes('nikkah') || event.name.toLowerCase().includes('nikah') || te.name.includes('نکاح')) && (
                                <div className="mb-3 inline-flex items-center gap-2 px-3 py-1.5 rounded border" style={{ backgroundColor: getOpacityStyle('bg', 0.1), borderColor: theme.accent, boxShadow: `0 0 10px ${theme.accent}33` }}>
                                  <span className="text-[10px] font-semibold" style={{ color: theme.accentLight }}>{s.language === 'ur' ? 'نکاح کی باقاعدہ رجسٹریشن کی جائے گی' : 'Nikah will be formally registered'}</span>
                                </div>
                              )}
