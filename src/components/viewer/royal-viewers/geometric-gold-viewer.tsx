...
export default function GeometricGoldViewer({ templateId, flowData, guestName, guestSlug, isReviewMode }: RoyalViewerProps) {
  ...
  return (
    ...
    >
      ...
      >
            ...
            <div className="max-w-lg mx-auto text-center">
              {s.guestNameFromUrl && (
                <m.div initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="flex flex-col items-center gap-4 mb-8">
                  <h3 className="text-3xl md:text-4xl capitalize font-[var(--font-cinzel-dec)]" style={{ color: theme.accent }}>{s.language === 'ur' ? 'محترم' : 'Dear'} {s.translatedGuestName},</h3>
        ...
        {/* ─── VIDEO ─── */}
        {s.youtubeVideoId && (<RevealSection><section className="py-16 md:py-20 px-6"><div className="flex flex-col items-center gap-6"><h2 className="text-3xl sm:text-4xl text-center font-[var(--font-cinzel-dec)] uppercase tracking-wider" style={{ color: theme.accent }}>{s.t('Our Story', 'Our Story')}</h2><GoldLineDivider accent={theme.accent} /><div className="w-full max-w-3xl mx-auto border" style={{ borderColor: getOpacityStyle('border', 0.2) }}><div className="aspect-video"><iframe className="w-full h-full" src={`https://www.youtube.com/embed/${s.youtubeVideoId}?rel=0&modestbranding=1`} title={s.t('Our Story', 'Our Story')} frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen /></div></div></div></section></RevealSection>)}
                ...
                {(guestName || s.guestNameFromUrl) && flowData?.guestSeats != null && (
                  <div className="flex flex-col items-center gap-2 mb-4">
                    <m.div
                      ...
                      className="flex items-center justify-center gap-2 px-5 py-2.5 border"
                    ...
                    >
                      ...
                      <span className="text-xs font-mono uppercase tracking-widest" style={{ color: theme.accent }}>
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 px-3.5 py-1.5 text-xs font-mono uppercase tracking-widest border transition-all hover:scale-105 active:scale-95 shadow-md cursor-pointer"
                    ...
                    {s.rsvpStatus === 'accept' && (
                      <div className="mt-4">
                        <button
                          type="button"
                          onClick={() => setIsPassModalOpen(true)}
                          className="inline-flex items-center gap-1.5 px-4 py-2 font-mono uppercase tracking-wider text-xs font-bold border transition-all hover:scale-105 active:scale-95 shadow-md cursor-pointer"
        ...
        <div className="py-10 text-center border-t" style={{ borderColor: getOpacityStyle('border', 0.1) }}>
          ...
          <p className="text-xs tracking-widest font-mono uppercase" style={{ color: getOpacityStyle('text', 0.5) }}>
